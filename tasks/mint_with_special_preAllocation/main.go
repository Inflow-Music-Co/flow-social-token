...
func main() {
	...
	flow.TransactionFromFile("fusd/mint_fusd").SignProposeAndPayAs("first").UFix64Argument("10000.00").AccountArgument("first").RunPrintEventsFull()
	...
	// Inflow Wallet Address
	flow.TransactionFromFile("social_token/set_fee_splitter").SignProposeAndPayAs("account").AccountArgument("first").UFix64Argument("0.03").RunPrintEventsFull()
	...
	flow.TransactionFromFile("social_token/setup_social_vault").SignProposeAndPayAs("second").RunPrintEventsFull()
	...
	// Update Special PreAllocation Structure
	flow.TransactionFromFile("social_token/update_special_preAllocation").SignProposeAndPayAs("account").AccountArgument("second").UFix64Argument("10000.0").UFix64Argument("1000.0").UIntArgument(5).RunPrintEventsFull()
	//Get the mintQoute for the specificed amount of socialTokens
	mintQuote := flow.ScriptFromFile("get_social_mint_quote").UFix64Argument("9999.999").RunFailOnError()
	log.Printf(" ------ MINT QUOTE ----- %s", mintQuote)

	flow.TransactionFromFile("social_token/mint_social_token").SignProposeAndPayAs("first").UFix64Argument("9999.999").UFix64Argument("6.6666").RunPrintEventsFull()

	// Get the balance of Actor account
	ArtistBalance := flow.ScriptFromFile("get_social_balance").AccountArgument("second").RunFailOnError()
	log.Printf(" ------ Artist Account Balance ----- %s", ArtistBalance)

	//Get the mintQoute for the specificed amount of socialTokens
	mintQuote = flow.ScriptFromFile("get_social_mint_quote").UFix64Argument("1.1").RunFailOnError()

	log.Printf(" ------ MINT QUOTE ----- %s", mintQuote)

	flow.TransactionFromFile("social_token/mint_social_token").SignProposeAndPayAs("first").UFix64Argument("1.1").UFix64Argument("1.1").RunPrintEventsFull()
